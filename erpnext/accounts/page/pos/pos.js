...
erpnext.pos.PointOfSale = erpnext.taxes_and_totals.extend({
	...
	init_master_data: function (r) {
		...
		this.make_control()
		console.log(this.batch_no_data);
	},
	...
	bind_items_event: function() {
		...
		$(this.wrapper).on('click', '.pos-bill-item', function() {
			...
			me.item_batch = $(this).attr("data-item-batch");
			me.render_selected_item()
			me.bind_qty_event()
			me.update_rate()
	...
	bind_qty_event: function () {
		...
		$(this.wrapper).on("change", ".pos-item-qty", function () {
			...
			var item_batch = $(this).parents(".pos-selected-item-action").attr("data-item-batch");
			var qty = $(this).val();
			me.update_qty(item_code, item_batch, qty);
			me.update_value();
		})
	...
	update_qty: function (item_code, item_batch, qty) {
		var me = this;
		this.items = this.get_items(item_code);
		this.validate_serial_no()
		this.set_item_details(item_code, item_batch, "qty", qty);
	},
	...
	update_rate: function () {
		...
		$(this.wrapper).on("change", ".pos-item-price", function () {
			var item_code = $(this).parents(".pos-selected-item-action").attr("data-item-code");
			var item_batch = $(this).parents(".pos-selected-item-action").attr("data-item-code");
			me.set_item_details(item_code, item_batch, "rate", $(this).val());
			me.update_value()
		})
	},
	...
	update_value: function() {
		...
		this.child_doc = this.get_child_item(this.item_code, this.item_batch);
	...
	get_child_item: function(item_code, item_batch) {
		var me = this;
		return $.map(me.frm.doc.items, function(doc){
			if(doc.item_code == item_code && doc.batch_no == item_batch) {
				return doc
			}
		})
	},

	set_item_details: function (item_code, item_batch, field, value) {
		var me = this;
		if (value < 0) {
		...
		$.each(this.frm.doc["items"] || [], function (i, d) {
			if (d.item_code == item_code && d.batch_no == item_batch) {
	...
	bind_change_batch: function() {
		var me = this;

		$(this.wrapper).on('click', '.change-batch', function () {
			var itc = $(this).parents('.pos-selected-item-action').attr("data-item-code");
			var ba = $(this).parents('.pos-selected-item-action').attr("data-item-batch");
			console.log(ba);
			me.select_batch(itc);
	...
	make_batch: function (itc) {
		var me = this;
		console.log(this.batch_no_data[itc]);
		$(this.$body).empty();
		$(this.$body).html(frappe.render_template('select_batch', {item_code:itc, batch_no:this.batch_no_data[itc]}));
		this.bind_batch_event(itc);

	},

	bind_batch_event: function(itc){
  	var me = this;


		$(this.$body).find('.batch_click').click(function(){
			var batch_no = $(this).val();
			var sp = 0;
			$.each(me.batch_no_data[itc] || [], function (i, d) {
					if (d['batch_id'] == batch_no) {
						sp = d['batch_price'];
		...
		$.each(me.frm.doc["items"] || [], function (i, d) {
			...
			if (sp != 0){
			me.set_item_details(itc, me.item_batch, "rate", sp);
			}
			else{
			me.set_item_details(itc, me.item_batch, "rate", me.price_list_data[itc]);
			}
			me.update_value();
			$(me.selected_row).find('.pos-batch').val(batch_no);
	...
	validate_batch_qty: function() {
		var me = this;
		var qty = 0;
		if (this.items[0].has_batch_no == 1){
			$.each(this.batch_no_data[this.items[0].item_code] || [], function(j,k){
				qty += k['actual_qty'];

			});
		}
		if (this.items[0].has_batch_no == 1 && qty == 0){
			frappe.throw(__("No Stock Batch"));
		}
	},
	...
	add_to_cart: function () {
		...
		// batch works
		this.validate_batch_qty();
		if (this.items[0].has_batch_no && !this.item_batch_no[this.items[0].item_code]) {
			if (no_of_items != 0) {
				$.each(this.frm.doc["items"] || [], function (i, d) {
					if (d.item_code == me.items[0].item_code) {
						$.each(me.batch_no_data[me.items[0].item_code] || [], function(j,k){
							if(d.batch_no == k['batch_id']){
								batch = k['batch_id'];
								caught1 = true;
							}
							else if (d.batch_no != k['batch_id'] && k['actual_qty'] > 0 && d.qty < k['actual_qty']) {
								batch = k['batch_id'];
								caught1 = true;
							}
						});
			...
			if(caught1 == false){
			frappe.throw(__("No Stock"));
			}
			else{
			this.item_batch_no[this.items[0].item_code] = batch;

		}
		}
		else{
			this.item_batch_no[this.items[0].item_code] = "None"
		}

   // batch work ends
		if (no_of_items != 0) {
			$.each(this.frm.doc["items"] || [], function (i, d) {
				if (d.item_code == me.items[0].item_code && d.batch_no == me.item_batch_no[d.item_code]) {
					caught = true;
					d.qty += 1;
					d.amount = flt(d.rate) * flt(d.qty);
	...
	show_items_in_item_cart: function () {
		...
		$.each(this.frm.doc.items || [], function (i, d) {
			var batch = "No";
			$.each(me.batch_no_data[d.item_code] || [], function (j, e){
				if(e['batch_price'] > 0){batch = "Yes"}
			});
	...
	mandatory_batch_no: function () {
		var me = this;
		console.log(this.batch_no_data[this.items[0].item_code]);
		if (this.items[0].has_batch_no && !this.item_batch_no[this.items[0].item_code]) {
	...
	get_actual_qty: function (item) {
		this.actual_qty = 0.0;
		var batch_qty = 0.0;
		$.each(this.batch_no_data[item.item_code] || [], function(j,k){
				batch_qty += k['actual_qty'];
