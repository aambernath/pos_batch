...
def get_batch_no_data(pos_profile):
	# get itemwise batch no data
	# exmaple: {'LED-GRE': [Batch001, Batch002]}
	# where LED-GRE is item code, SN0001 is serial no and Pune is warehouse
	# we need to only send items with valid batch - we need not send items that has been expired
	itemwise_batch = {}
	batches = frappe.db.sql("""select name, item, expiry_date, batch_id, batch_price from `tabBatch`
		where ifnull(expiry_date, '4000-10-10') >= curdate() order by expiry_date asc""", as_dict=1)
	w = pos_profile.get('warehouse')
	for batch in batches:
		ste = get_batch_qty(batch.batch_id, w, batch.item)
		# check for batch with qty > 0 and in same warehouse
		if batch.item not in itemwise_batch:
			itemwise_batch.setdefault(batch.item, [])
		itemwise_batch[batch.item].append({'batch_id':batch.batch_id, 'expiry':batch.expiry_date, 'price':batch.batch_price, 'actual_qty':ste['actual_batch_qty']})
